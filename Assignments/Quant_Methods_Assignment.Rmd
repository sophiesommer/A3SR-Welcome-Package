...
# T-test Recap     
#### One Sample T-test     
For a sample of size $n$, we can estimate the standard error of $\bar{x}$ using $SE(\bar{x})=\frac{s}{\sqrt{n}}$ where $s$ is the sample standard deviation.  

This can be used to derive a T Score, which can then be compared to a t distribution with $n-1$ degrees of freedom.
...
Suppose for a moment that you only have one set of heights, say, $\{60, 63, 63, 70, 72\}$ and you want to find some value, $M$, such that the sum of the differences between $M$ and each value in your set of heights is minimized. It turns out that the value of $M$ which satisfies this minimization problem is the mean of the dataset (you can prove this with some relatively simple calculus). The proof is left to the interested reader, but hopefully this claim already feels somewhat intuitive.  
