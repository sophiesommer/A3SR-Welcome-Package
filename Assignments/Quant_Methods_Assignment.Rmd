...
Given sample mean $\bar{X}$ and the standard error of the sample mean, $SE(\bar{x})$, we can construct a t-test statistic: $$T = \frac{\bar{x}-\mu_0}{SE(\bar{x})}$$ The value of this T-test statistic can be compared to a t distribution with $n-1$ degrees of freedom.

If abs(t) is greater than t(0.975, n-1), we can reject the null hypothesis and conclude that the population mean is different from $\mu_0$. 

Note that for a sample size of $n$ the $SE(\bar{x})=\frac{s}{\sqrt{n}}$ where $s$ is the sample standard deviation. 
...
We can construct a t statistic to test $H_0$ versus $H_a$, $$T = \frac{(\bar{x_1}-\bar{x_2})}{SE(\bar{x_1}-\bar{x_2})}$$
...
Suppose for a moment that you only have one set of heights, say, $\{60, 63, 63, 70, 72\}$ and you want to find some value, $M$, such that the sum of the squared differences between $M$ and each value in your set of heights is minimized. It turns out that the value of $M$ which satisfies this minimization problem is the mean of the dataset (you can prove this with some relatively simple calculus). The proof is left to the interested reader, but hopefully this claim already feels somewhat intuitive.  
...
# Save mean height of women as MM
