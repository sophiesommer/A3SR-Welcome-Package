...
```{r}
run10_2 = run10 %>% select(time, pace, age, gender, state) %>% na.omit()
```
...
```{r}
run10_2 = run10_2 %>% mutate(fivek_split = pace * 3.10686)
```
  
4. Now, calculate mean 5k split times for each gender group   
```{r}
run10_2 %>% group_by(gender) %>% summarise(Mean_5k_Split = mean(fivek_split)) 
```
...
```{r}
run10_2 = run10_2 %>% mutate(decade = as.factor(floor(age/10))) 
```
...
```{r}
run10_2 %>% filter(gender=="F") %>% arrange(time) %>% select(state) %>% head(10) %>% unique()
run10_2 %>% filter(gender=="M") %>% arrange(time) %>% select(state) %>% head(10)
```
...
run10_2 %>% 
  ...
  group_by(decade) %>% 
  summarise(median_finish_time = median(time_hours))
```
...
```{r}
run10_2 = left_join(run10_2, state_data, by = "state") 
```
...
```{r}
run10_2 %>% filter(number_from_state>=50 & number_from_state<=200) %>% 
  group_by(state) %>% 
