...
## What is the difference between a true probability and what we observe in simulation?
...
Prob.Flip1Coin = function(n) {
  ...
  return(sum(x)/n)
  # this line calculates the observed proportion of heads
}
```
...
# This is to ensure that everyone gets the same results (the choice of 12345 was random)
set.seed(12345)
...
for (i in 1:nIter){
  #insert code here to roll two die and take their sum, save the results as a variable called "Roll"
  
  if(Roll==7){
    #insert code here to save (in Earning[i]) the amount that you will earn if the sum of your rolls is 7 
  }
  #insert "else if" and "else" statements here to record earnings for each other possible outcome of Roll
}

```
  
Answer here: Should you keep playing the game?  
